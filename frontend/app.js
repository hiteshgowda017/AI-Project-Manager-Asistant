...
const App = (() => {
  ...
  const ui = {
    const App = (() => {
      const apiBaseUrl = "https://ai-pm-backend-0lfc.onrender.com/api/v1";
      ...
      const ui = {
        ...
        setApiStatus(isOnline) {
          ...
          const dot = elements.apiStatusInline.querySelector(".dot");
          const label = elements.apiStatusInline.querySelector("span:last-child");
          if (label) {
      ...
      const actions = {
        ...
        async commitTaskDrafts() {
          if (!state.activeProjectId) return;
          if (state.taskDrafts.length === 0) return;
      ...
      function cacheElements() {
        ...
        elements.workspaceDescription = document.getElementById(
      ...
      function bindEvents() {
        ...
        if (saveWorkspaceButton) {
          saveWorkspaceButton.addEventListener("click", async () => {
            try {
              ui.setButtonLoading(saveWorkspaceButton, true, "Saving...");
              await actions.createWorkspace();
            } catch (error) {
              ui.toast(error.message);
            } finally {
              ui.setButtonLoading(saveWorkspaceButton, false);
            }
          });
        }
        ...
        if (saveProjectButton) {
          saveProjectButton.addEventListener("click", async () => {
            try {
              ui.setButtonLoading(saveProjectButton, true, "Saving...");
              await actions.createProject();
            } catch (error) {
              ui.toast(error.message);
            } finally {
              ui.setButtonLoading(saveProjectButton, false);
            }
          });
        }
        if (createTaskButton) {
        ...
        if (saveTaskButton) {
          saveTaskButton.addEventListener("click", async () => {
            try {
              ui.setButtonLoading(saveTaskButton, true, "Saving...");
              await actions.createTask();
            } catch (error) {
              ui.toast(error.message);
            } finally {
              ui.setButtonLoading(saveTaskButton, false);
            }
          });
        }
        if (refreshInsightsButton) {
          refreshInsightsButton.addEventListener("click", async () => {
            try {
              ...
              ui.setButtonLoading(refreshInsightsButton, true, "Refreshing...");
              await api.refreshInsights(state.activeProjectId);
              await actions.loadInsights();
            } catch (error) {
              ui.toast(error.message);
            } finally {
              ui.setButtonLoading(refreshInsightsButton, false);
            }
          });
        }
        if (loadHealthButton) {
          loadHealthButton.addEventListener("click", async () => {
            try {
              ui.setButtonLoading(loadHealthButton, true, "Loading...");
              await actions.loadHealth();
            } catch (error) {
              ui.toast(error.message);
            } finally {
              ui.setButtonLoading(loadHealthButton, false);
            }
          });
        }
        if (generateTasksButton) {
          generateTasksButton.addEventListener("click", async () => {
            try {
              ui.setButtonLoading(generateTasksButton, true, "Generating...");
              await actions.generateTaskDrafts();
            } catch (error) {
              ui.toast(error.message);
            } finally {
              ui.setButtonLoading(generateTasksButton, false);
            }
          });
        }
        if (commitTasksButton) {
          commitTasksButton.addEventListener("click", async () => {
            try {
              ui.setButtonLoading(commitTasksButton, true, "Committing...");
              await actions.commitTaskDrafts();
            } catch (error) {
              ui.toast(error.message);
            } finally {
              ui.setButtonLoading(commitTasksButton, false);
            }
          });
        }
        if (askDecisionButton) {
          askDecisionButton.addEventListener("click", async () => {
            try {
              ui.setButtonLoading(askDecisionButton, true, "Analyzing...");
              await actions.createDecision();
            } catch (error) {
              ui.toast(error.message);
            } finally {
              ui.setButtonLoading(askDecisionButton, false);
            }
          });
        }
      }
